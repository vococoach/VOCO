// Curated, hand-written SAT vocabulary content.
// No AI calls at runtime — this is the entire content library for the MVP.
//
// Structure: categories (organized by FUNCTION, matching how the real SAT
// tests vocab — agreement, disagreement, degree, etc.) > levels (difficulty
// tiers within a category) > words.
//
// Quiz format matches the real Digital SAT "Words in Context" question type:
// a sentence with a blank, four plausible answer choices, and only one that
// precisely fits the tone and logic of that specific sentence — NOT a plain
// "what does this word mean" question.
//
// STATUS: "Agreement & Support" is fully built (34 words across 3 levels).
// The other 5 categories are stubbed with just a title/description — their
// `levels` array is empty until content is added, so the home screen shows
// them as "Coming soon."
...
// study/quiz pages so routes don't need to know which category a level
// belongs to.
...
// Every word across every category/level, flattened into one list with its
// id and the category/level it belongs to attached. Used by the review
// screen, which pulls due words from anywhere in the library.

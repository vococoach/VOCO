...
export function getTonight(courses, sets, now, isLocked) {
  ...
  // `levels` is every unlocked level (so studying an optional extra tonight
  // still counts as tonight's study being done); `core` leaves out the optional
  // extras (level.optional — the SAT Expert tier). Suggestions only ever come
  // from `core`: an extra is opt-in from the course page, and must not change
  // what tonight's study suggests, or when a course counts as used up.
  const perCourse = courses.map((course) => {
    const levels = unlockedLevels(course.categories, isLocked);
    return { course, levels, core: levels.filter(({ level }) => !level.optional), lastStudied: courseLastStudied(course, sets) };
  });
  ...
  for (const p of started) {
    const next = p.core.find(isUnstudied);
    if (next) return { kind: "suggest", ...next };
  }

  const unstarted = perCourse.filter((p) => p.lastStudied === null && p.core.length > 0).map((p) => p.course);
  if (unstarted.length > 0) return { kind: "choose", courses: unstarted };

  let oldest = null;
  perCourse.flatMap((p) => p.core).forEach(({ category, level }) => {

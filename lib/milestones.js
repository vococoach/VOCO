...
// The levels that count toward mastery and the course summary: every level
// except the optional extras (`optional: true` — the SAT Expert tier, an extra on
// top of the original three). Keeping extras out is what lets a tier be added to
// a category without un-mastering anyone who had already mastered it.
export function requiredLevels(category) {
  return category.levels.filter((level) => !level.optional);
}

export function getMasteredCategories(categories, sets) {
  return categories.filter((category) => {
    const required = requiredLevels(category);
    return required.length > 0 && required.every((level) => isPerfectOnce(sets[level.id]));
  });
}
...
export function getCourseProgress(course, sets) {
  const withLevels = course.categories.filter((category) => requiredLevels(category).length > 0);
  let levelsTotal = 0;
  let levelsCompleted = 0;
  withLevels.forEach((category) =>
    requiredLevels(category).forEach((level) => {
      levelsTotal += 1;

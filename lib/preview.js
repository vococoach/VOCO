// The single free sample question offered for each locked category (in any
// course), so a visitor can see the real quiz format before paying.
//
...
import { categories, getCategoryCourse } from "./wordbanks";
import { isFreeCategory } from "./purchase";

export function getPreviewSample(categoryId) {
  const category = categories.find((c) => c.id === categoryId);
  // Only the paid categories have previews — each course's free one is just free.
  if (!category || isFreeCategory(category.id) || category.levels.length === 0) return null;
  const word = category.levels[0].words[0];
  const totalQuestions = category.levels.reduce((n, level) => n + level.words.length, 0);
  return { course: getCategoryCourse(category.id), category, word, totalQuestions };
}

...
export async function renderShareCard(card) {
  ...
  // --- headline + note (wrapped) ---
  ctx.font = `600 54px ${FONT_DISPLAY}`;
  ctx.fillStyle = "#3D2B4F";
  const headlineLines = wrapLines(ctx, card.headline, cardW - 140);
  let y = unitY + 92;
  ...
  y += 14;
  for (const line of wrapLines(ctx, card.note, cardW - 160)) {
    ctx.fillText(line, cx, y);
    y += 42;
  }
  ...
  ctx.fillText("Study at night. Quiz in the morning.", cx, cardY + cardH + 88);
  ctx.fillStyle = "#8B85FF";
  ctx.font = `600 42px ${FONT_TEXT}`;
  spacedText(ctx, "voco.courses", cx, cardY + cardH + 168, 5);
